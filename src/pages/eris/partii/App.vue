...
<h1 id="code-from-previous-post">Code from Previous Post</h1>
<p>As promised, I will be putting the code here for those who just want to grab and go:
...
<h1 id="the-meme-command-a-name-cmd-a-">The <code>Meme</code> Command <a name="cmd"></a></h1>
<p>Now, for the <code>Meme</code> command! For this, we will need to get the memes from reddit. For that, we will be using <code>got</code> to get the JSON from <code>https://www.reddit.com/r/memes/random/.json</code>.
Install <code>got</code> first:</p>
<pre><code class="lang-bash">npm <span class="hljs-selector-tag">i</span> got --save
</code></pre>
<p>Create a file in <code>./commands</code> and name it <code>meme.js</code>.
Put the following code inside (I will be explaining it later):</p>
